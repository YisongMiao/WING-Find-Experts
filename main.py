...
def main():
    ...
    query_file = f"data-query/test_query_{args.query_index}.json"
    ...
    try:
        with open("data-AE/{}".format(args.profile_path), "r") as f:
            database = json.load(f)
    except FileNotFoundError:
        print("Author profile not found. Please contact our team to get the author profile file.")
        exit()
    ...
    mode_dir = os.path.join(args.output_path, args.author_embedding)
    os.makedirs(mode_dir, exist_ok=True)
    
    # Save fitness scores to CSV in descending order
    csv_file = os.path.join(mode_dir, f"fitness_scores_query_{args.query_index}.csv")
    ...
    output_file = os.path.join(mode_dir, f"output_query_{args.query_index}.txt")
